...
gcloud beta container --project $DEVSHELL_PROJECT_ID clusters create $NAME --zone $ZONE  --image-type "UBUNTU" --scopes "https://www.googleapis.com/auth/cloud-platform" --metadata startup-script='#! /bin/bash \sudo apt-get update -y \ sudo apt install wget \ sudo mkdir /tmp/world wget https://storage.cloud.google.com/$DEVSHELL_PROJECT_ID-$REGION-$NAME/$WORLD /tmp/world EOF',disable-legacy-endpoints=true
